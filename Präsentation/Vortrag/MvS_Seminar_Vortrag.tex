...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{block}{\textit{cheating-Husband}-Erweiterung}
			\begin{itemize}
				\item Königin Henrietta II beerbt Henrietta I
				\item Einführung des Mail-Systems
...
\begin{frame}
	\frametitle{ Wissen in asynchronen Systemen}
		...
		\begin{block}{Satz}
			Wenn es mehr als einen untreuen Ehemann gibt und diese Nachricht über einen asynchronen Kanal verbreitet wird, so wird kein Ehemann erschossen.
		\end{block}
		
		\begin{block}{Beweis (logisch)}
			\begin{itemize}
				\item Durch asynchrones Versenden gibt es ein \textit{eventuelles} Gemeinsames Wissen
				\item Der Brief der Königin erreicht unter Umständen ihr Ziel
				\item Die Adressatin des Briefes kann nicht sicher sein, dass andere Ehefrauen ebenfalls einen Brief erhalten haben
				\item So kann keine Frau aus \textit{ruhigen} Nächten schließen, dass Ihr Ehemann der Betrüger ist.
				
				\item Ähnlichkeit => \textit{Problem der Byzantinischen Generäle}
			\end{itemize}
		\end{block}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{block}{\textit{cheating-Husband}-Erweiterung}
			\begin{itemize}
				\item Königin Henrietta III beerbt Henrietta II
				\item Upgrade des Mail-Systems
				\item Jeder Brief soll nach Versand maximal 1 Tag zum Ziel benötigen
			\end{itemize}
		\end{block}	
		
		\begin{block}{b-common-knowledge}
			Innerhalb von b Tagen erhält jede Frau einen Brief und weiß, dass innerhalb von b Tagen jede Frau über das gleiche Wissen verfügt.
		\end{block}
	\end{itemize}
\end{frame}
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{block}{Satz}
			In schwachen synchronen Systemen mit einer Grenze b beim Ausliefern der Briefe, weiß eine Frau exakt von k untreuen Männern, wenn $k*b$ stille Nächte nach dem Erhalt des Briefs vorüber gegangen sind.
		\end{block}
	\end{itemize}
\end{frame}
\begin{frame}
	...
	\item starke synchrone Systeme :
		\begin{block}{\textit{cheating-Husband}-Erweiterung}
			\begin{itemize}
				\item Königin Henrietta IV beerbt Henrietta III
				\item Einführen des Kalenders
				\item Jeder Brief soll nach b Tagen zugestellt sein
				\item Frauen verlieren vertrauen in die Königin
				\item Zunächst wurde 1000 Nächte kein Mann mehr erschossen
				\item Trick:
					\begin{itemize}
						\item Neuer Brief der Königin
...
\begin{frame}
	\frametitle{ Übersicht - Gemeinsames Wissen (asynchrone Systeme)}
	\begin{itemize}
		\item Es muss eine Vereinbarung zwischen den Prozessen getroffen werden.
		\item Gibt keine Simultaneität 
		\item Es gibt abgeschwächte Formen, die es einfacher machen:
		\begin{itemize}
		\item Epsilon gemeinsames Wissen
		\item Eventuelles gemeinsames Wissen
		\item Zeitabhängiges gemeinsames Wissen
...
\begin{frame}
	...
	\begin{block}{Beschreibung}
	Die Prozesse treffen eine Vereinbarung in einem globalen Status der Ausführung. Diese kann, muss aber nicht, konsistent sein.
	\end{block}	
	\begin{block}{Definition}
	\begin{itemize}
...
\begin{frame}
	\frametitle{ Zeitabhängiges gemeinsames Wissen }
	
	\begin{block}{Beschreibung}
		Prozesse erreichen Vereinbarungen mit einem Status, der die gleiche lokale Zeit hat. 
		(Sind alle Uhren immer perfekt synchronisiert, so entspricht das Ergebnis regulär dem gemeinsamen Wissen.)
	\end{block}	
	\begin{block}{Definition}
	\begin{itemize}
...
\begin{frame}
	\frametitle{ Gleichzeitiges gemeinsames Wissen }
		...
		\begin{block}{Beschreibung}
		Prozesse erreichen lokal einen konsistenten Status, welcher zu einem konsistenten Schnittpunkt gehört. Wenn Prozess $P_i$ gemeinsames Wissen von $\phi$ erlangen will, so hat Prozess $P_j$ dasselbe Wissen in einem konsistenten Status.
	\end{block}	
...
\begin{frame}
  \frametitle{Zusammenfassung }
	...
	\begin{itemize}
		...
		\item In asynchronen Systemen kann nicht immer bewerkstelligt werden, dass die gewünschte Nachricht beim Empfänger ankommt.
		\item Je nach Anwendungsfall gibt es Möglichkeiten für asynchrone Systeme "'synchrone Aktionen"' bewerkstelligen zu können.
